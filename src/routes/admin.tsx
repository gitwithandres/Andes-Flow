...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Panel de control de inventarios, producción y oportunidades de ANDES." },
    ],
  }),
  component: AdminPage,
});

type TabType = "solicitudes" | "tiempos" | "productos" | "materiales" | "oportunidades" | "comentarios";
...
function AdminPage() {
  ...
  const openAddModal = () => {
    ...
    setFormData({
      id: `SOL-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
      institution: "",
  ...
  const handleSaveRequest = async () => {
    if (editingRequest) {
      setRequests(prev => prev.map(r => r.id === editingRequest.id ? formData as ProductionRequest : r));
      await supabase.from('production_requests').update({
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [reqs, times, prods, mats, opps, coms] = await Promise.all([
        ...
        supabase.from('product_ratings').select('*, production_requests(institution, resource)').order('created_at', { ascending: false })
      ]);

      if (reqs.data) {
        setRequests(reqs.data.map((d: any) => ({
          id: d.id,
      ...
      if (times.data) {
        setTiempos(times.data.map((d: any) => ({
          id: d.id,
      ...
      if (prods.data) {
        setProductos(prods.data.map((d: any) => ({
          id: d.id,
      ...
      if (mats.data) {
        setMateriales(mats.data.map((d: any) => ({
          id: d.id,
          name: d.name,
          type: d.type,
          color: d.color,
      ...
      if (opps.data) {
        setOportunidades(opps.data.map((d: any) => ({
          id: d.id,
  ...
  const filteredComentarios = useMemo(() => {
    return comentarios.filter((c) => {
      ...
      return (
        ...
        (c.production_requests?.institution && c.production_requests.institution.toLowerCase().includes(q)) ||
        (c.production_requests?.resource && c.production_requests.resource.toLowerCase().includes(q))
      );
    });
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Sistema integral de seguimiento de insumos, tiempos de producción, y oportunidades de venta de ANDES.
            </p>
          </div>
      ...
      <div className="mb-6 overflow-x-auto flex border-b border-border">
        <TabButton active={activeTab === "solicitudes"} onClick={() => setActiveTab("solicitudes")} label="Solicitudes de Producción" badge={requests.length} />
        <TabButton active={activeTab === "tiempos"} onClick={() => setActiveTab("tiempos")} label="Tiempos de Producción" />
        <TabButton active={activeTab === "productos"} onClick={() => setActiveTab("productos")} label="Productos Terminados" />
        <TabButton active={activeTab === "materiales"} onClick={() => setActiveTab("materiales")} label="Inventario de Materiales" />
        <TabButton active={activeTab === "oportunidades"} onClick={() => setActiveTab("oportunidades")} label="Oportunidades de Venta" />
        <TabButton active={activeTab === "comentarios"} onClick={() => setActiveTab("comentarios")} label="Comentarios" badge={comentarios.length} />
      </div>
      ...
      {activeTab === "solicitudes" && (
        <div className="space-y-6 animate-in fade-in-50 duration-200">
          <div className="grid gap-4 md:grid-cols-5">
            <SummaryCard title="Recibidas" count={requestsSummary["Recibida"]} className="border-blue-200 bg-blue-50/30 text-blue-900" />
            <SummaryCard title="En Producción" count={requestsSummary["En Produccion"]} className="border-yellow-200 bg-yellow-50/30 text-yellow-900" />
            <SummaryCard title="Revisión Calidad" count={requestsSummary["Revision de Calidad"]} className="border-purple-200 bg-purple-50/30 text-purple-900" />
            <SummaryCard title="Empaque & Transp." count={requestsSummary["Empaque y Transporte"]} className="border-orange-200 bg-orange-50/30 text-orange-900" />
            <SummaryCard title="Entregadas" count={requestsSummary["Entregada"]} className="border-green-200 bg-green-50/30 text-green-900" />
          </div>
                ...
                {filteredRequests.map((r) => (
                  <TableRow key={r.id} className="hover:bg-muted/10">
                    ...
                    <TableCell>
                      <select
                        value={r.status}
                        onChange={(e) => handleStatusChange(r.id, e.target.value as ProductionRequest["status"])}
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => openEditModal(r)} title="Editar">
                          <Edit2 className="h-4 w-4" />
                        </Button>
                        <Button variant="outline" size="icon" className="h-8 w-8 text-red-500 hover:text-red-700 hover:bg-red-50" onClick={() => handleDeleteRequest(r.id)} title="Eliminar">
          ...
          <Dialog open={isRequestModalOpen} onOpenChange={setIsRequestModalOpen}>
            <DialogContent className="sm:max-w-[500px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingRequest ? "Modifica los datos de la solicitud existente." : "Ingresa los datos para registrar una nueva solicitud de producción."}
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  <Label htmlFor="institution">Institución</Label>
                  <Input id="institution" placeholder="Ej. Colegio San José" value={formData.institution} onChange={(e) => setFormData({...formData, institution: e.target.value})} />
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="resource">Recurso Solicitado</Label>
                  <Input id="resource" placeholder="Ej. Figura Sistema Solar" value={formData.resource} onChange={(e) => setFormData({...formData, resource: e.target.value})} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="quantity">Cantidad</Label>
                    <Input id="quantity" type="number" min={1} value={formData.quantity} onChange={(e) => setFormData({...formData, quantity: Number(e.target.value)})} />
                  </div>
                  <div className="grid gap-2">
                ...
                <div className="grid gap-2">
                  ...
                  <Input id="responsible" placeholder="Nombre del solicitante" value={formData.responsible} onChange={(e) => setFormData({...formData, responsible: e.target.value})} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label htmlFor="modal-priority">Prioridad</Label>
                    <select id="modal-priority" className="rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.priority} onChange={(e) => setFormData({...formData, priority: e.target.value as any})}>
                      <option value="Alta">Alta</option>
                      <option value="Media">Media</option>
                      <option value="Baja">Baja</option>
                    </select>
                  ...
                  <div className="grid gap-2">
                    ...
                    <select id="modal-status" className="rounded-md border border-input bg-background px-3 py-2 text-sm" value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value as any})}>
                      <option value="Recibida">Recibida</option>
                      <option value="En Produccion">En Producción</option>
                      <option value="Revision de Calidad">Revisión de Calidad</option>
                      <option value="Empaque y Transporte">Empaque y Transporte</option>
                ...
                <div className="grid gap-2">
                  ...
                  <Input id="progress" type="number" min={0} max={100} value={formData.progress_percentage || 0} onChange={(e) => setFormData({...formData, progress_percentage: Number(e.target.value)})} />
                </div>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setIsRequestModalOpen(false)}>Cancelar</Button>
                <Button onClick={handleSaveRequest}>{editingRequest ? "Guardar Cambios" : "Crear Solicitud"}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
              ...
              <TableBody>
                {filteredTiempos.map((t) => (
                  <TableRow key={t.id} className="hover:bg-muted/10">
                    ...
                    <TableCell className="font-medium text-foreground">{t.resourceName}</TableCell>
                    <TableCell className="text-center">
                      <Badge variant="outline">{t.material}</Badge>
                    </TableCell>
                    <TableCell className="text-right">{Number(t.printTimeHours).toFixed(1)}</TableCell>
                    <TableCell className="text-right">{Number(t.postProcessHours).toFixed(1)}</TableCell>
                    <TableCell className="text-right font-bold text-accent">{Number(t.totalTimeHours).toFixed(1)}</TableCell>
                    <TableCell className="text-right">{t.speedMms}</TableCell>
                    <TableCell className="text-right">{t.partsPerUnit}</TableCell>
                    <TableCell className="text-right text-muted-foreground">{Number(t.timePerBatch5Hours).toFixed(1)}</TableCell>
                  </TableRow>
                ))}
                
                {/* Averages Row matches screenshot exact values */}
                <TableRow className="bg-muted/50 font-bold border-t-2 border-primary/20">
                  <TableCell colSpan={2} className="text-left uppercase tracking-wider text-primary">Promedio General</TableCell>
                  <TableCell className="text-center">-</TableCell>
                  <TableCell className="text-right text-primary">{tiemposAverages.print.toFixed(1)} h</TableCell>
                  <TableCell className="text-right text-primary">{tiemposAverages.post.toFixed(1)} h</TableCell>
                  <TableCell className="text-right text-accent font-extrabold">{tiemposAverages.total.toFixed(1)} h</TableCell>
                  <TableCell colSpan={3} className="text-right text-muted-foreground font-normal italic">
              ...
              <TableBody>
                {filteredProductos.map((p) => (
                  <TableRow key={p.id} className="hover:bg-muted/10">
                    ...
                    <TableCell className="text-right font-semibold text-green-600">{p.available}</TableCell>
                ...
                <TableRow className="bg-muted/50 font-bold border-t-2 border-primary/20">
                  <TableCell colSpan={3} className="text-left uppercase tracking-wider text-primary">Totales</TableCell>
                  ...
                  <TableCell className="text-right text-green-600">{productosTotals.available}</TableCell>
                  <TableCell className="text-right text-yellow-600">{productosTotals.inProduction}</TableCell>
                  <TableCell className="text-right text-purple-600">{productosTotals.reserved}</TableCell>
                  <TableCell className="text-center">-</TableCell>
                </TableRow>
              </TableBody>
            </Table>
              ...
              <TableBody>
                {filteredMateriales.map((m) => (
                  <TableRow key={m.id} className="hover:bg-muted/10">
                    ...
                    <TableCell className="text-right font-bold">{Number(m.stockKg).toFixed(1)}</TableCell>
                    <TableCell className="text-right text-muted-foreground">{Number(m.minStockKg).toFixed(1)}</TableCell>
                    <TableCell className="text-right text-muted-foreground">{Number(m.maxStockKg).toFixed(1)}</TableCell>
              ...
              <TableBody>
                {filteredOportunidades.map((o) => (
                  <TableRow key={o.id} className="hover:bg-muted/10">
                    ...
                    <TableCell className="text-xs text-muted-foreground">{o.subcategory}</TableCell>
                    <TableCell className="text-xs">{o.potentialPublic}</TableCell>
                    <TableCell className="text-right font-bold text-accent">{o.referencePriceCop}</TableCell>
                    <TableCell className="text-center">
                      <Badge className={
                        o.complexity === "Alta"
                          ? "bg-red-100 text-red-700 hover:bg-red-100 border-red-200"
                    ...
                    <TableCell className="text-xs text-muted-foreground max-w-[280px] truncate" title={o.observation}>
                      {o.observation}
                    </TableCell>
                  </TableRow>
                ))}
            ...
            <Table>
              ...
              <TableBody>
                {filteredComentarios.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No hay comentarios aún.</TableCell>
                  </TableRow>
                ) : (
                  filteredComentarios.map((c) => (
                    <TableRow key={c.id} className="hover:bg-muted/10">
                      ...
                      <TableCell className="font-medium text-foreground">{c.production_requests?.institution || "N/A"}</TableCell>
                      ...
                      <TableCell className="italic text-muted-foreground max-w-sm truncate" title={c.feedback}>

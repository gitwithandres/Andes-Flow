    ...
    <>
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-4 md:grid-cols-2 md:items-center">
          <div className="max-w-2xl text-white">
            ...
            <h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl leading-[1.1]">
              ...
              <span className="bg-clip-text text-transparent bg-linear-to-r from-primary via-primary/80 to-accent">al alcance de todos.</span>
            </h1>
            <p className="mt-6 text-lg text-slate-200 leading-relaxed drop-shadow-sm">
              Plataforma integral que democratiza el acceso a material educativo especializado y ayudas técnicas. Mediante visualización 3D interactiva en tiempo real y fabricación digital optimizada, facilitamos el proceso de solicitud y seguimiento para instituciones y comunidades de todo el país.
            </p>
            <div className="mt-10 flex flex-wrap items-center gap-4">
              <Link to="/catalogo" className={buttonVariants({ size: "lg", className: "h-14 px-8 text-base font-bold shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all bg-primary hover:bg-primary/95 text-primary-foreground border-0" })}>
                Explorar marketplace <ArrowRight className="ml-2 w-5 h-5" />
              </Link>
              <Link to="/admin" className={buttonVariants({ variant: "outline", size: "lg", className: "h-14 px-8 text-base font-semibold bg-white/10 hover:bg-white/20 border-white/20 text-white backdrop-blur-sm" })}>
                Acceso corporativo
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 lg:gap-6 relative">
            {[
              { icon: Boxes, label: "Inventario en tiempo real", desc: "Gestión y control de stock centralizado" },
              { icon: Layers3, label: "Visor 3D interactivo", desc: "Inspección 360° y Realidad Aumentada" },
              { icon: HandHeart, label: "Impacto y equidad social", desc: "Diseñado para la inclusión educativa" },
              { icon: ShieldCheck, label: "Seguridad y trazabilidad", desc: "Monitoreo completo de solicitudes" },
            ].map((f) => (
              <Card key={f.label} className="border-white/10 bg-black/40 backdrop-blur-md text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-primary/50">
      ...
      <section className="py-20 bg-secondary/30 border-y border-border/50">
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-12 gap-4">
            <div>
              <h2 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Materiales Destacados</h2>
              <p className="mt-3 text-lg text-muted-foreground max-w-2xl">
                Explora los recursos más solicitados e interactúa con sus modelos en 3D para comprender a detalle su diseño anatómico y funcional antes de realizar tu pedido.
              </p>
            </div>
            <Link to="/catalogo" className="inline-flex items-center font-semibold text-primary hover:text-primary/80 transition-colors group">
              Ver todo el inventario <ArrowRight className="ml-1 w-4 h-4 transition-transform group-hover:translate-x-1" />
            </Link>
          </div>
          
          <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {featuredMaterials.map((m) => (
              <Card key={m.id} className="group flex flex-col overflow-hidden border-border/40 bg-background shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:border-primary/30">
      ...
      <section className="py-24 relative">
        <div className="mx-auto max-w-7xl px-4 text-center">
          <h2 className="text-3xl font-extrabold tracking-tight sm:text-4xl mb-16">¿Cómo funciona ANDES?</h2>
